...
ipcRenderer.on('crop_data', async (event, arg) => {
    ...
    video.addEventListener('readystatechange', function() {
        ...
        if(video.readyState === 4 && haveRunCropperStuff === false) {
            ...
            doAllCropperStuff(baseCamCrop, baseScreenCrop, video, isScreenvas, callback, canvas, clip);
      ...
      video.addEventListener('loadeddata', function() {
          ...
          if(video.readyState > 0 && haveRunCropperStuff === false) {
            haveRunCropperStuff = true;
            doAllCropperStuff(baseCamCrop, baseScreenCrop, video, isScreenvas, callback, canvas, image, clip);
          }
      });
...
let doAllCropperStuff = (baseCamCrop, baseScreenCrop, video, isScreenvas, callback, canvas, image, clip) => {
    ...
    const doneFunc = () => {
        ...
        ipcRenderer.send('camvas_closed', JSON.stringify(
            {
                cropDetails: {
    ...
    let screenvasDataProcessor = (event, arg) => {
        ...
        console.log(`${JSON.stringify(camData) + JSON.stringify(camCrop) + JSON.stringify(screenCrop)}`);
